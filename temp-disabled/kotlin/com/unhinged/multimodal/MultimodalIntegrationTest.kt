...
class MultimodalIntegrationTest {
    ...
    private fun createMockVisionClient(): VisionAIGrpcClient {
        // Create a mock that simulates successful gRPC responses
        return object : VisionAIGrpcClient("localhost", 50051, 60) {
            override suspend fun infer(
                ...
                maxTokens: Int,
                temperature: Float
            ): com.unhinged.application.multimodal.VisionInferenceResult {
                return com.unhinged.application.multimodal.VisionInferenceResult(
                    description = "Mock analysis: This is a test $analysisType with UI elements",
                    confidence = 0.85,
                    modelUsed = model,
                    processingTime = 1.5,
                    metadata = mapOf("mock" to "true", "model_used" to model),
                    extractedText = if (analysisType == "document") "Sample extracted text" else null,
                    uiElements = if (analysisType == "screenshot" || analysisType == "ui_component") {
                        listOf(
                            com.unhinged.application.multimodal.VisionUIElement(
                                type = "button",
                                confidence = 0.9,
                                bounds = com.unhinged.application.multimodal.VisionElementBounds(200, 200, 100, 30),
                                properties = mapOf("text" to "Submit")
                            )
                        )
                    } else emptyList(),
                    tags = listOf("ui", "interface", "test")
                )
            }
        }
    }
    ...
    private fun createMockContextClient(): ContextLLMGrpcClient {
        return object : ContextLLMGrpcClient("localhost", 50052, 30) {
            override suspend fun generatePrompt(
                ...
                return "Enhanced prompt: $basePrompt with context for $analysisType analysis"
            }
        }
    }

    private fun createAnalysisUseCase(): com.unhinged.application.multimodal.MultimodalAnalysisUseCase {
        return com.unhinged.application.multimodal.MultimodalAnalysisUseCase(
    ...
    private fun createGetAnalysisUseCase(): com.unhinged.application.multimodal.GetAnalysisUseCase {
        return com.unhinged.application.multimodal.GetAnalysisUseCase(repository)
    }

    private fun createGetUserAnalysesUseCase(): com.unhinged.application.multimodal.GetUserAnalysesUseCase {
        return com.unhinged.application.multimodal.GetUserAnalysesUseCase(repository)
    }

    private fun createSearchAnalysesUseCase(): com.unhinged.application.multimodal.SearchAnalysesUseCase {
        return com.unhinged.application.multimodal.SearchAnalysesUseCase(repository)
    }

    private fun createGetStatisticsUseCase(): com.unhinged.application.multimodal.GetAnalysisStatisticsUseCase {
        return com.unhinged.application.multimodal.GetAnalysisStatisticsUseCase(repository)
    }

    private fun createMockContextService(): com.unhinged.domain.multimodal.ContextService {
        return object : com.unhinged.domain.multimodal.ContextService {
            override suspend fun searchContext(
                query: String,
                ...
                return listOf(
                    ContextItem(
                        ...
                        filePath = "/mock/path/doc.md",
                        tags = listOf("test", "mock"),
                        relevanceScore = 0.8
                    )
                )
            }

            override suspend fun getContextByType(contextType: ContextType, limit: Int): List<ContextItem> {
                return searchContext("", listOf(contextType), limit)
            }

            override suspend fun refreshContext(): Int = 1
        }
    }

    // Additional mock services needed
    private fun createMockDomainService(): com.unhinged.domain.multimodal.MultimodalDomainService {
        return object : com.unhinged.domain.multimodal.MultimodalDomainService {
            ...
            override fun selectOptimalWorkflow(analysisType: AnalysisType, priority: Priority, userPreference: WorkflowType?): WorkflowConfiguration {
                return workflowSelector.selectOptimalWorkflow(analysisType, priority, userPreference)
            }
            ...
            override fun generateCacheKey(request: AnalysisRequest): String = "cache-${request.imageId}-${request.analysisType}"
    ...
    private fun createMockVisionService(): com.unhinged.application.multimodal.VisionInferenceService {
        return object : com.unhinged.application.multimodal.VisionInferenceService {
            override suspend fun infer(
                imageData: ByteArray,
                model: String,
                prompt: String,
                ...
                return createMockVisionClient().infer(imageData, model, prompt, analysisType, maxTokens, temperature)
            }
        }
    }

    private fun createMockContextLLMService(): com.unhinged.application.multimodal.ContextLLMService {
        return object : com.unhinged.application.multimodal.ContextLLMService {
            override suspend fun generatePrompt(
                basePrompt: String,
                analysisType: String,
                contextTypes: List<String>,
                ...
                return createMockContextClient().generatePrompt(basePrompt, analysisType, contextTypes, maxContextItems)
            }
        }
    }

    private fun createMockCacheService(): com.unhinged.application.multimodal.CacheService {
        private val cache = mutableMapOf<String, Any>()

        return object : com.unhinged.application.multimodal.CacheService {
