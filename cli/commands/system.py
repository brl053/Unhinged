...
def start():
    ...
    if not check_file_exists("libs/python/service_framework/service_launcher.py", "Service launcher"):
        return 1

    result = run_command([python_cmd, "libs/python/service_framework/service_launcher.py", "--timeout", "120"])
    if result == 0:
...
def status():
    ...
    run_command([python_cmd, "libs/python/service_framework/service_launcher.py", "--status"])
