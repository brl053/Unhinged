...
async def _query_with_reasoning(query_text: str, execute: bool, dry_run: bool) -> dict[str, object]:
...
def query(prompt, file, output, execute, dry_run, explain):  # type: ignore[override]
    ...
    try:
        if explain:
            result_dict = asyncio.run(_query_with_reasoning(query_text, execute, dry_run))
        else:
            plan = build_audio_volume_plan(query_text)
