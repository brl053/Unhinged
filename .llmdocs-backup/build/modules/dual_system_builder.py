...
class DualSystemBuilder(BuildModule):
    ...
    def can_handle(self, target_name: str) -> bool:
        ...
        dual_system_targets = {
            'dual-system-desktop', 'dual-system-build', 'dual-system-package',
            'conversation-cli', 'conversation-cli-test', 'desktop-app-enhanced',
    ...
    def validate_environment(self) -> List[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                errors.append("GTK4 and Libadwaita are REQUIRED - install with: sudo apt install gir1.2-gtk-4.0 gir1.2-adw-1")
        except Exception:
    ...
    def build(self, target_name: str) -> BuildModuleResult:
        ...
        if env_errors:
            error_msg = f"Dual-system build environment validation failed:\n" + '\n'.join(f"  - {error}" for error in env_errors)
        ...
        try:
            ...
            return BuildModuleResult(
                ...
                duration=duration,
                artifacts=[],
                error_message=f"Dual-system build failed: {str(e)}"
            )
    ...
    def _build_desktop_application(self) -> List[BuildArtifact]:
        ...
        artifacts.append(BuildUtils.create_build_artifact(
            desktop_app_dst, "executable",
            {"type": "desktop_application", "enhanced": True}
        ))
    ...
    def _build_conversation_cli(self) -> List[BuildArtifact]:
        ...
        artifacts.append(BuildUtils.create_build_artifact(
            cli_dst, "executable",
            {"type": "conversation_cli", "voice_first": True}
        ))
    ...
    def _build_distribution_packages(self) -> List[BuildArtifact]:
        ...
        if package_script.exists():
            result = subprocess.run([
                "bash", str(package_script), "all"
            ], cwd=self.desktop_dir, capture_output=True, text=True)
                ...
                if dist_desktop.exists():
                    for package_file in dist_desktop.rglob("*"):
                        if package_file.is_file():
                            ...
                            artifacts.append(BuildUtils.create_build_artifact(
                                dst_file, "distribution_package",
                                {"package_type": package_file.suffix}
                            ))
    ...
    def _test_conversation_cli(self) -> List[BuildArtifact]:
        ...
        if result.returncode == 0:
            ...
            artifacts.append(BuildUtils.create_build_artifact(
                ...
                {"test_type": "conversation_cli", "status": "passed"}
            ))
    ...
    def _test_dual_system_integration(self) -> List[BuildArtifact]:
        ...
        desktop_app = self.desktop_dir / "unhinged-desktop-app"
        result = subprocess.run([
            "python3", "-m", "py_compile", str(desktop_app)
        ], capture_output=True, text=True)
        ...
        with open(test_report, 'w') as f:
            ...
            f.write(f"Native C Graphics: {'AVAILABLE' if (self.context.project_root / 'libs' / 'graphics' / 'build' / 'libunhinged_graphics.so').exists() else 'MISSING'}\n")
            f.write(f"Conversation CLI: {'AVAILABLE' if (self.control_dir / 'conversation_cli.py').exists() else 'MISSING'}\n")
            f.write("Status: INTEGRATION_READY\n")
        
        artifacts.append(BuildUtils.create_build_artifact(
            test_report, "test_report",
            {"test_type": "dual_system_integration", "status": "passed"}
        ))
