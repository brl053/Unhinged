/**
 * @file bitmap_font.c
 * @brief Simple 8x8 bitmap font for text rendering
 * 
 * Hard-coded bitmap font data for basic ASCII characters.
 ...
 */
ug_error_t ug_draw_char(ug_surface_t* surface, int32_t x, int32_t y, char c, ug_color_t color) {
        ...
        for (int col = 0; col < 8; col++) {
            if (row_data & (0x80 >> col)) {
                ...
                if (px >= 0 && py >= 0 && px < surface->width && py < surface->height) {
                    ...
                    surface->pixels[py * surface->width + px] = pixel;
                }
            }
        }
    }
 ...
 */
ug_error_t ug_draw_text(ug_surface_t* surface, int32_t x, int32_t y, const char* text, ug_color_t color) {
    if (!surface || !text) {
        return UG_ERROR_INVALID_PARAM;
    }
    ...
    while (*text) {
        if (*text == '\n') {
            ...
            ug_draw_char(surface, current_x, current_y, *text, color);
            current_x += 8;
        }
        text++;
    }
