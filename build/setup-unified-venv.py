...
class UnifiedVenvManager:
    ...
    def _install_dependencies(self):
        ...
        subprocess.run([str(pip_path), "install", "-r", str(self.unified_requirements)], check=True)
    ...
    def _install_local_packages(self):
        ...
        if event_framework_path.exists():
            subprocess.run([str(pip_path), "install", "-e", str(event_framework_path)], check=True)
    ...
    def _create_activation_helpers(self):
        ...
        if run_script.exists():
            ...
            content = content.replace("build/python/venv", ".venv")
    ...
    def _setup_development_tools(self):
        ...
        try:
            ...
            if hooks_script.exists():
                subprocess.run(["bash", str(hooks_script)], cwd=self.project_root, check=True, capture_output=True)
