    ...
    def _analyze_static_references(self):
        ...
        for file_path in self.project_root.rglob("*"):
            if (
                ...
                and not any(excl in str(file_path) for excl in self.exclusions)
            ):
                try:
    ...
    def _identify_unused_scripts(self):
        ...
        for script_dir in script_dirs:
            ...
            for script_file in script_dir.rglob("*.py"):
                ...
                referenced = (
                    str(relative_path) in self.makefile_references
                    or str(relative_path) in self.build_config_references
                    or any(str(relative_path) in ref for ref in self.static_references)
                )
