...
class PortValidator:
    ...
    def _suggest_alternative_port(self, conflicted_port: int) -> int:
        ...
        alternatives = [conflicted_port + 1, conflicted_port + 10, conflicted_port + 100, conflicted_port + 1000]
    ...
    def generate_fix_script(self, conflicts: list[PortConflict]) -> str:
        ...
        if files_to_backup:
            backup_commands.extend(
                ["# Backup all affected docker-compose files", "BACKUP_TIMESTAMP=$(date +%Y%m%d_%H%M%S)", ""]
            )
