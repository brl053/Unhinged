                ...
                if match.is_file() and match.name not in allowed_root_files:
                    results.append(
                        ValidationResult(
                            ...
                            severity="ERROR",
                            message="Forbidden file in root directory",
                            file_path=match,
                            fix_suggestion="Move to appropriate subdirectory or remove",
                            auto_fixable=False,
                            category="pattern",
                        )
                    )
                ...
                if ".git" not in str(match) and "venv" not in str(match):
                    results.append(
                        ValidationResult(
                            validator_name=self.name,
                            severity="WARNING",
                            message="Backup/temp file found",
                            file_path=match,
            ...
            for match in self.repo_root.rglob(pattern):
                ...
                results.append(
                    ValidationResult(
                        validator_name=self.name,
                        severity="WARNING",
                        message="Generated file outside /generated/",
                        file_path=match,
...
class PolyglotValidationRunner:
    ...
    def _register_default_validators(self):
        ...
        try:
            from .python_validator import PythonFormatterValidator, PythonValidator

            self.validators.extend([PythonValidator(self.repo_root), PythonFormatterValidator(self.repo_root)])
        except ImportError:
            pass
...
async def main():
    ...
    if args.format == "json":
        ...
        # Text format
        print("🔍 Polyglot Validation Results")
        print("=" * 50)
