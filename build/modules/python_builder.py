...
class PythonBuilder(BuildModule):
    ...
    def can_handle(self, target_name: str) -> bool:
        ...
        return (
            target_name in python_targets
            or "python" in target_name
            or any(service in target_name for service in ["whisper-tts", "vision-ai", "context-llm"])
        )
        ...
        for service_dir in service_dirs:
            if service_dir.exists():
                src_hash = BuildUtils.calculate_directory_hash(
                    service_dir, patterns=["*.py", "requirements.txt", "pyproject.toml"]
                )
    ...
    def build(self, target_name: str) -> BuildModuleResult:
        ...
        if not build_steps:
            return BuildModuleResult(
                success=False, duration=0.0, artifacts=[], error_message=f"Unknown Python target: {target_name}"
            )
        ...
        return BuildModuleResult(
            success=True, duration=duration, artifacts=all_artifacts, warnings=all_warnings, metrics=all_metrics
        )
    ...
    def _get_build_steps(self, target_name: str) -> list[tuple]:
        ...
        for service_dir in service_dirs:
            ...
            if (service_dir / "requirements.txt").exists():
                steps.append((f"Install {service_name} dependencies", "pip3 install -r requirements.txt", service_dir))
            elif (service_dir / "pyproject.toml").exists():
                steps.append((f"Install {service_name} dependencies", "pip3 install .", service_dir))

            # Run tests if this is a test target
            if "test" in target_name:
                if (service_dir / "tests").exists():
                    steps.append((f"Run {service_name} tests", "python3 -m pytest tests/ -v", service_dir))
                elif (service_dir / "test.py").exists():
                    steps.append((f"Run {service_name} tests", "python3 test.py", service_dir))

            # Build package if this is a build target
            if "build" in target_name and (service_dir / "setup.py").exists():
                steps.append((f"Build {service_name} package", "python3 setup.py build", service_dir))
        ...
        for service_dir in service_dirs:
            ...
            if dist_dir.exists():
                for wheel_file in dist_dir.glob("*.whl"):
                    artifacts.append(
                        BuildUtils.create_build_artifact(wheel_file, "python_wheel", {"service": service_dir.name})
                    )

                for tar_file in dist_dir.glob("*.tar.gz"):
                    artifacts.append(
                        BuildUtils.create_build_artifact(tar_file, "python_sdist", {"service": service_dir.name})
                    )
                ...
                for report_file in service_dir.glob("test-report*.xml"):
                    artifacts.append(
                        BuildUtils.create_build_artifact(
                            report_file, "test_report", {"format": "junit", "service": service_dir.name}
                        )
                    )
