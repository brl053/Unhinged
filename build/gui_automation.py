...
class GUIAutomation:
    ...
    def __init__(self, app_name: str = "unhinged", log_dir: Path | None = None):
    ...
    def click(self, x: int, y: int) -> GUIResult:
        ...
        try:
            ...
            result = GUIResult(
                success=False, action=GUIAction(ActionType.CLICK, value=f"{x},{y}").to_dict(), error=str(e)
            )
            self._log_action(result)
            return result
    ...
    def type_text(self, text: str, delay: float = 0.05) -> GUIResult:
        ...
        try:
            for char in text:
                ...
                time.sleep(delay)
            result = GUIResult(
                success=True, action=GUIAction(ActionType.TYPE, value=text).to_dict(), output=f"Typed: {text}"
            )
            self._log_action(result)
            return result
        except Exception as e:
            result = GUIResult(success=False, action=GUIAction(ActionType.TYPE, value=text).to_dict(), error=str(e))
            self._log_action(result)
            return result
    ...
    def wait(self, seconds: float) -> GUIResult:
        ...
        try:
            ...
            result = GUIResult(
                success=False, action=GUIAction(ActionType.WAIT, value=str(seconds)).to_dict(), error=str(e)
            )
            self._log_action(result)
            return result
    ...
    def find_window(self, window_name: str) -> GUIResult:
        """Find window by name"""
        try:
            output = subprocess.check_output(["xdotool", "search", "--name", window_name], text=True).strip()
            result = GUIResult(
                success=bool(output),
            ...
            result = GUIResult(
                success=False, action=GUIAction(ActionType.FIND_WINDOW, target=window_name).to_dict(), error=str(e)
            )
            self._log_action(result)
            return result
    ...
    def focus_window(self, window_id: str) -> GUIResult:
        ...
        try:
            ...
            result = GUIResult(
                success=False, action=GUIAction(ActionType.FOCUS_WINDOW, target=window_id).to_dict(), error=str(e)
            )
            self._log_action(result)
            return result
