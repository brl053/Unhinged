...
if [[ "$*" == *"--no-verify"* ]]; then
    echo ""
    echo "🚫 --no-verify DETECTED - Quality gates cannot be bypassed"
    exit 1
fi

if [[ -n "$SKIP" ]]; then
    echo ""
    echo "🚫 SKIP DETECTED: $SKIP - Quality gates cannot be bypassed"
    exit 1
fi
...
'''
    ...
    def is_installed(self) -> bool:
        ...
        """
        return (
            self.pre_commit_hook.exists()
            and os.access(self.pre_commit_hook, os.X_OK)
        )
