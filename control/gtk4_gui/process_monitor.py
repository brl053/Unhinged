...
class ProcessMonitor:
    ...
    def get_process_list(self, include_system: bool = True) -> List[ProcessInfo]:
        ...
        try:
            ...
            # Get all processes
            for proc in psutil.process_iter(['pid', 'name', 'username', 'status', 'create_time', 'ppid', 'num_threads']):
                try:
                    # Get basic process info
                    pinfo = proc.info
                    
                    # Skip system processes if requested
                    if not include_system and pinfo['username'] in ['root', 'system', 'daemon']:
                        continue
                    ...
                    logger.debug(f"Error collecting info for process {pinfo.get('pid', 'unknown')}: {e}")
                    self._error_count += 1
                    continue
    ...
    def _get_cpu_percent(self, proc) -> float:
        ...
        try:
            ...
            if (pid in self._cpu_percent_cache and 
                current_time - self._cpu_percent_cache[pid]['time'] < 1.0):
    ...
    def _is_critical_process(self, proc) -> bool:
        ...
        try:
            ...
            critical_names = {
                ...
                'networkmanager', 'systemd-', 'gdm', 'lightdm'
            }
...
if __name__ == "__main__":
    ...
    for proc in top_cpu:
        print(f"  {proc.name} (PID: {proc.pid}): {proc.cpu_percent:.1f}% CPU, {proc.memory_percent:.1f}% Memory")
